    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.53)', height: '100vh' }}>
            <div style={{ width: '400px', padding: '20px', borderRadius: '6px', background: '#fff' }}>
                ...
                />
                {message && <span style={{ color: message.includes('Hướng dẫn') ? 'green' : 'red' }}>{message}</span>}
                <Loading isPending={isPending}>
                    <ButtonComponent
                        disabled={!email.length || isPending}
                        onClick={handleResetPassword}
                        size={40}
                        styleButton={{
                            background: isPending ? '#ccc' : '#e30019',
                            height: '48px',
                            width: '100%',
                            border: 'none',
                            borderRadius: '4px',
                        ...
                        textbutton={isPending ? 'Đang xử lý...' : 'Reset Password'}
                        styletextbutton={{ color: '#fff', fontSize: '20px', fontWeight: '700' }}
                    />
                </Loading>
                <WrapperTextLight onClick={() => navigate('/sign-in')}>Đăng nhập</WrapperTextLight>
